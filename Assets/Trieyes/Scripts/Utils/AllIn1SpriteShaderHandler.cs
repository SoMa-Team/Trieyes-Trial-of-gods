...
namespace Utils
{
    public enum AllIn1SpriteShaderType
    {
        None,
        DamageNormal,
        DamageCritical,
    ...
    public class AllIn1SpriteShaderHandler
    {
        public Material mat;
        private static readonly string[] allToggleProperties = new string[]
        {
            "GLOW_ON",
            "FADE_ON",
            "OUTLINE_ON",
        ...
        public void SetShaderAllObjects(AllIn1SpriteShaderType type)
        {
            switch(type)
            {
                ...
                // Fade Amount 0.5로 주고 Fade 옵션 켜기
                    ...
                    break;
                case AllIn1SpriteShaderType.DamageCritical:
                    break;
                case AllIn1SpriteShaderType.DamageOverTime:
                    break;
                case AllIn1SpriteShaderType.DamageBurn:
                    break;
                case AllIn1SpriteShaderType.DamageFreeze:
                    break;
                case AllIn1SpriteShaderType.DamageElectricShock:
                    break;
                case AllIn1SpriteShaderType.Death:
                    break;
                case AllIn1SpriteShaderType.Off:
                    // 모든 Effect 비활성화
                    DisableAllEffects();
                    break;
            }
        ...
        private void SetAllEffects(bool on)
        {
            if (mat == null) return;
            for (int i = 0; i < allToggleProperties.Length; i++)
            {
                string prop = allToggleProperties[i];
                if (on) mat.EnableKeyword(prop);
                else mat.DisableKeyword(prop);
            }
        }
